...
export function createHexFloat(
  ...
  function createTarget(
    ...
    return { fbo, height: size, texture, width: size }
  }
  ...
  function releaseSim() {
    ...
    simPrograms.forEach((p) => {
      gl!.deleteProgram(p)
    })
    simShaders.forEach((s) => {
      gl!.deleteShader(s)
    })
  }
  ...
  function stepSim(delta: number) {
    ...
    gl!.uniform1f(clearProgram.uniforms.uValue!, PRESSURE_DECAY ** (delta * 60))
    blit(pressure.write)
    pressure.swap()
  ...
  function createPostTarget(width: number, height: number): Target {
    ...
    return { fbo, height, texture, width }
  }
  ...
  function onSelMove(event: MouseEvent) {
    ...
    const sel = window.getSelection()
    if (caret?.node && sel?.anchorNode && content.contains(caret.node)) {
