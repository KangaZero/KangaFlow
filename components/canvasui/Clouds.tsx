...
export function createClouds(
  ...
  function link(fragSource: string) {
    ...
    const uniform = (name: string): WebGLUniformLocation | null =>
      uniforms[name] ?? null
    return { fs, program, uniform, uniforms, vs }
  }
  ...
  function render(delta: number) {
    ...
    gl!.uniform2f(field.uniform("uResolution"), fieldW, fieldH)
    gl!.uniform2f(
      field.uniform("uOffset"),
      content.scrollLeft / Math.max(content.clientWidth, 1),
      -content.scrollTop / Math.max(content.clientHeight, 1)
    )
    gl!.uniform1f(field.uniform("uTime"), time)
    gl!.uniform1f(field.uniform("uScale"), Math.max(config.scale, 0.05))
    gl!.uniform1f(field.uniform("uCover"), Math.max(config.cover, 0))
    gl!.uniform1f(field.uniform("uDensity"), Math.max(config.density, 0))
    gl!.drawArrays(gl!.TRIANGLE_STRIP, 0, 4)

    const prevWind = windTextures[windIndex]
    const nextWind = windTextures[windIndex === 0 ? 1 : 0]
    windIndex = windIndex === 0 ? 1 : 0
    gl!.framebufferTexture2D(
      gl!.FRAMEBUFFER,
      gl!.COLOR_ATTACHMENT0,
      gl!.TEXTURE_2D,
    ...
    gl!.uniform1i(windPass.uniform("uPrev"), 0)
    gl!.uniform2f(windPass.uniform("uResolution"), fieldW, fieldH)
    gl!.uniform1f(windPass.uniform("uDecay"), 0.5 ** (delta / 0.7))
    const moved = Math.hypot(pointerX - prevPointerX, pointerY - prevPointerY)
    const stamping = hasPointer && moved > 0
    gl!.uniform2f(windPass.uniform("uA"), prevPointerX, prevPointerY)
    gl!.uniform2f(windPass.uniform("uB"), pointerX, pointerY)
    gl!.uniform1f(
      windPass.uniform("uRadius"),
      Math.max(config.windRadius, 1) / Math.max(output.clientHeight, 1)
    )
    gl!.uniform1f(
      windPass.uniform("uStrength"),
    ...
    gl!.activeTexture(gl!.TEXTURE0)
    gl!.bindTexture(gl!.TEXTURE_2D, fieldTexture)
    gl!.uniform1i(composite.uniform("uField"), 0)
    gl!.activeTexture(gl!.TEXTURE1)
    gl!.bindTexture(gl!.TEXTURE_2D, contentTexture)
    gl!.uniform1i(composite.uniform("uContent"), 1)
    gl!.activeTexture(gl!.TEXTURE2)
    gl!.bindTexture(gl!.TEXTURE_2D, nextWind)
    gl!.uniform1i(composite.uniform("uWind"), 2)
    gl!.uniform2f(composite.uniform("uResolution"), output.width, output.height)
    gl!.uniform2f(
      composite.uniform("uContentScale"),
      contentScaleX,
      contentScaleY
    )
    gl!.uniform3f(
      composite.uniform("uBase"),
      baseColor[0],
      baseColor[1],
      baseColor[2]
    )
    gl!.uniform1f(
      composite.uniform("uOpacity"),
      Math.min(Math.max(config.opacity, 0), 1)
    )
    gl!.uniform1f(composite.uniform("uShading"), Math.max(config.shading, 0))
    gl!.uniform1f(
      composite.uniform("uShadow"),
      Math.min(Math.max(config.shadow, 0), 1)
    )
    gl!.uniform2f(
      composite.uniform("uShadowShift"),
      -config.shadowOffsetX / Math.max(output.clientWidth, 1),
      config.shadowOffsetY / Math.max(output.clientHeight, 1)
    )
    gl!.uniform1f(
      composite.uniform("uShadowLod"),
      Math.min(Math.max(config.shadowSoftness, 0), 1) * 4
    )
    gl!.uniform1f(
      composite.uniform("uWindAmt"),
      Math.min(Math.max(config.wind, 0), 1)
    )
    gl!.uniform1f(
      composite.uniform("uRefraction"),
      Math.max(config.refraction, 0) / Math.max(output.clientWidth, 1)
    )
    gl!.uniform1f(
      composite.uniform("uFogBlur"),
      Math.min(Math.max(config.fogBlur, 0), 1)
    )
    gl!.uniform1f(composite.uniform("uHasContent"), htmlInCanvas ? 1 : 0)
    gl!.drawArrays(gl!.TRIANGLE_STRIP, 0, 4)
  }
