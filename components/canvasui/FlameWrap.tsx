...
export interface FlameWrapOptions {
  ...
  scorch?: number
  /** Global burn sweep 0 to 1. Drives the dissolve front across the whole
   * content — at 1 the content is consumed entirely, leaving flames + smoke. */
  blaze?: number
}
...
void main () {
  ...
  float blaze = max(uBlaze, 0.0);
  float dn = fbm2(rel * (3.2 / unit) * detail + vec2(0.0, t * 0.5) + 91.0);
  float dw = mix(2.0, 5.0, wTop) * (1.0 + blaze * 14.0);
  float dissolve = S(
    -dw,
    dw,
    front + (dn - 0.5) * dw * 2.5 + blaze * (uRectHalf.y * 2.0 + unit)
  );
...
export function createFlameWrap(
  ...
  function render() {
    ...
    gl!.uniform1f(uniforms.uBlaze!, Math.max(config.blaze, 0))
...
export interface FlameWrapProps extends FlameWrapOptions {
  ...
  /** Live handle to the running effect (for animations via setOptions). */
  ref?: Ref<FlameWrapInstance>
}
...
export function FlameWrap({
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      setOptions: (next) => instanceRef.current?.setOptions(next),
    }),
    []
  )
