...
export interface GlassOptions {
  /**
   * Static pane: covers the whole element and never follows the cursor. Used
   * as a fixed glass background (e.g. behind a window's content) instead of an
   * interactive lens.
   */
  fill?: boolean
  /** Lens shape. */
...
export function createGlass(
  ...
  const fill = Boolean(config.fill)
  let presence = fill ? 1 : 0
  let presenceTarget = fill ? 1 : 0
  let targetX = posX
  let targetY = posY
  let zoom = 1
  let zoomTarget = 1
  ...
  function halfExtents(): [number, number] {
    if (fill) {
      return [
        Math.max(8, content.clientWidth / 2),
        Math.max(8, content.clientHeight / 2),
      ]
    }
  ...
  function render() {
    ...
    gl!.uniform2f(uniform("uCenter"), cx, cy)
    gl!.uniform2f(uniform("uHalf"), halfW * dpr, halfH * dpr)
    const corner =
      config.shape === "circle"
        ? Math.min(halfW, halfH)
        : Math.min(Math.max(config.corner, 0), Math.min(halfW, halfH))
    gl!.uniform1f(uniform("uCorner"), corner * dpr)
    gl!.uniform1f(uniform("uEdge"), Math.min(Math.max(config.edge, 0), 0.98))
    gl!.uniform1f(uniform("uBevel"), Math.max(config.bevel, 0.5))
    gl!.uniform1f(uniform("uIor"), Math.min(Math.max(config.ior, 1.01), 2.5))
    gl!.uniform1f(uniform("uDepth"), Math.max(config.depth, 0) * dpr)
    gl!.uniform1f(uniform("uAberration"), Math.max(config.aberration, 0))
    gl!.uniform1f(uniform("uBlur"), Math.max(config.blur, 0))
    gl!.uniform1f(uniform("uReflect"), Math.max(config.reflection, 0))
    gl!.uniform1f(uniform("uShine"), Math.max(config.shine, 0))
    gl!.uniform1f(uniform("uZoom"), Math.max(zoom, 1))
