...
// Context so module-level component overrides can read the locale prop without
// being defined inside Calendar (which would violate noNestedComponentDefinitions).
const LocaleContext = React.createContext<Partial<Locale> | undefined>(
  undefined
)
...
function CalendarChevron({
  ...
  orientation,
  ...props
}: React.SVGAttributes<SVGElement> & { orientation?: string }) {
  if (orientation === "left") {
...
function CalendarDayButtonWrapper({
  ...
  return (
    <CalendarDayButton {...(locale != null ? { locale } : {})} {...props} />
  )
}
