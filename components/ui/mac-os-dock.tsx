"use client"

import type React from "react"
import { useCallback, useEffect, useRef, useState } from "react"
import { cn } from "@/lib/utils"

interface DockApp {
  id: string
  name: string
  icon: string
}

interface MacOSDockProps {
  apps: DockApp[]
  onAppClick: (appId: string) => void
  openApps?: string[]
  className?: string
}

// GSAP is loaded globally (script tag) when available; type only the `.to` call
// we use so the optional dependency stays type-safe without `any`.
type GsapTween = {
  to: (target: Element, vars: Record<string, unknown>) => void
}

const getGsap = (): GsapTween | undefined =>
  (globalThis as { gsap?: GsapTween }).gsap

const MacOSDock: React.FC<MacOSDockProps> = ({
  apps,
  onAppClick,
  openApps = [],
  className = "",
}) => {
  const [mouseX, setMouseX] = useState<number | null>(null)
  const [currentScales, setCurrentScales] = useState<number[]>(
    apps.map(() => 1)
  )
  const [currentPositions, setCurrentPositions] = useState<number[]>([])
  const dockRef = useRef<HTMLDivElement>(null)
  const iconRefs = useRef<(HTMLDivElement | null)[]>([])
  const animationFrameRef = useRef<number | undefined>(undefined)
  const lastMouseMoveTime = useRef<number>(0)

  const getResponsiveConfig = useCallback(() => {
    if (typeof window === "undefined") {
      return { baseIconSize: 64, effectWidth: 240, maxScale: 1.6 }
    }

    const smallerDimension = Math.min(window.innerWidth, window.innerHeight)

    if (smallerDimension < 480) {
      return {
        baseIconSize: Math.max(40, smallerDimension * 0.08),
        effectWidth: smallerDimension * 0.4,
        maxScale: 1.4,
      }
    }
    if (smallerDimension < 768) {
      return {
        baseIconSize: Math.max(48, smallerDimension * 0.07),
        effectWidth: smallerDimension * 0.35,
        maxScale: 1.5,
      }
    }
    if (smallerDimension < 1024) {
      return {
        baseIconSize: Math.max(56, smallerDimension * 0.06),
        effectWidth: smallerDimension * 0.3,
        maxScale: 1.6,
      }
    }
    return {
      baseIconSize: Math.max(64, Math.min(80, smallerDimension * 0.05)),
      effectWidth: 300,
      maxScale: 1.8,
    }
  }, [])

  const [config, setConfig] = useState(getResponsiveConfig)
  const { baseIconSize, maxScale, effectWidth } = config
  const minScale = 1.0
  const baseSpacing = Math.max(4, baseIconSize * 0.08)

  useEffect(() => {
    const handleResize = () => {
      setConfig(getResponsiveConfig())
    }

    window.addEventListener("resize", handleResize)
    return () => window.removeEventListener("resize", handleResize)
  }, [getResponsiveConfig])

  // biome-ignore lint/correctness/useExhaustiveDependencies: minScale is a stable literal kept in the list intentionally; redundant but documented.
  const calculateTargetMagnification = useCallback(
    (mousePosition: number | null) => {
      if (mousePosition === null) {
        return apps.map(() => minScale)
      }

      return apps.map((_, index) => {
        const normalIconCenter =
          index * (baseIconSize + baseSpacing) + baseIconSize / 2
        const minX = mousePosition - effectWidth / 2
        const maxX = mousePosition + effectWidth / 2

        if (normalIconCenter < minX || normalIconCenter > maxX) {
          return minScale
        }

        const theta = ((normalIconCenter - minX) / effectWidth) * 2 * Math.PI
        const cappedTheta = Math.min(Math.max(theta, 0), 2 * Math.PI)
        const scaleFactor = (1 - Math.cos(cappedTheta)) / 2

        return minScale + scaleFactor * (maxScale - minScale)
      })
    },
    [apps, baseIconSize, baseSpacing, effectWidth, maxScale, minScale]
  )

  const calculatePositions = useCallback(
    (scales: number[]) => {
      let currentX = 0

      return scales.map((scale) => {
        const scaledWidth = baseIconSize * scale
        const centerX = currentX + scaledWidth / 2
        currentX += scaledWidth + baseSpacing
        return centerX
      })
    },
    [baseIconSize, baseSpacing]
  )

  // biome-ignore lint/correctness/useExhaustiveDependencies: minScale and config are kept deliberately so the scales/positions re-initialize on resize; they are stable/redundant but documented as intentional.
  useEffect(() => {
    const initialScales = apps.map(() => minScale)
    const initialPositions = calculatePositions(initialScales)
    setCurrentScales(initialScales)
    setCurrentPositions(initialPositions)
  }, [apps, calculatePositions, minScale, config])

  const animateToTarget = useCallback(() => {
    const targetScales = calculateTargetMagnification(mouseX)
    const targetPositions = calculatePositions(targetScales)
    const lerpFactor = mouseX !== null ? 0.2 : 0.12

    setCurrentScales((prevScales) => {
      return prevScales.map((currentScale, index) => {
        const target = targetScales[index]
        if (target === undefined) {
          return currentScale
        }
        const diff = target - currentScale
        return currentScale + diff * lerpFactor
      })
    })

    setCurrentPositions((prevPositions) => {
      return prevPositions.map((currentPos, index) => {
        const target = targetPositions[index]
        if (target === undefined) {
          return currentPos
        }
        const diff = target - currentPos
        return currentPos + diff * lerpFactor
      })
    })

    const scalesNeedUpdate = currentScales.some((scale, index) => {
      const target = targetScales[index]
      return target !== undefined && Math.abs(scale - target) > 0.002
    })
    const positionsNeedUpdate = currentPositions.some((pos, index) => {
      const target = targetPositions[index]
      return target !== undefined && Math.abs(pos - target) > 0.1
    })

    if (scalesNeedUpdate || positionsNeedUpdate || mouseX !== null) {
      animationFrameRef.current = requestAnimationFrame(animateToTarget)
    }
  }, [
    mouseX,
    calculateTargetMagnification,
    calculatePositions,
    currentScales,
    currentPositions,
  ])

  useEffect(() => {
    if (animationFrameRef.current) {
      cancelAnimationFrame(animationFrameRef.current)
    }
    animationFrameRef.current = requestAnimationFrame(animateToTarget)

    return () => {
      if (animationFrameRef.current) {
        cancelAnimationFrame(animationFrameRef.current)
      }
    }
  }, [animateToTarget])

  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      const now = performance.now()

      if (now - lastMouseMoveTime.current < 16) {
        return
      }

      lastMouseMoveTime.current = now

      if (dockRef.current) {
        const rect = dockRef.current.getBoundingClientRect()
        const padding = Math.max(8, baseIconSize * 0.12)
        setMouseX(e.clientX - rect.left - padding)
      }
    },
    [baseIconSize]
  )

  const handleMouseLeave = useCallback(() => {
    setMouseX(null)
  }, [])

  const createBounceAnimation = (element: HTMLElement) => {
    const bounceHeight = Math.max(-8, -baseIconSize * 0.15)
    element.style.transition = "transform 0.2s ease-out"
    element.style.transform = `translateY(${bounceHeight}px)`

    setTimeout(() => {
      element.style.transform = "translateY(0px)"
    }, 200)
  }

  const handleAppClick = (appId: string, index: number) => {
    const iconEl = iconRefs.current[index]
    if (iconEl) {
      const gsap = getGsap()
      if (gsap) {
        const bounceHeight =
          (currentScales[index] ?? minScale) > 1.3
            ? -baseIconSize * 0.2
            : -baseIconSize * 0.15

        gsap.to(iconEl, {
          duration: 0.2,
          ease: "power2.out",
          repeat: 1,
          transformOrigin: "bottom center",
          y: bounceHeight,
          yoyo: true,
        })
      } else {
        createBounceAnimation(iconEl)
      }
    }

    onAppClick(appId)
  }

  const contentWidth =
    currentPositions.length > 0
      ? Math.max(
          ...currentPositions.map(
            (pos, index) =>
              pos + (baseIconSize * (currentScales[index] ?? minScale)) / 2
          )
        )
      : apps.length * (baseIconSize + baseSpacing) - baseSpacing

  const padding = Math.max(8, baseIconSize * 0.12)

  return (
    <div
      className={cn("backdrop-blur-md", className)}
      onMouseLeave={handleMouseLeave}
      onMouseMove={handleMouseMove}
      ref={dockRef}
      role="button"
      style={{
        background: "rgba(45, 45, 45, 0.75)",
        border: "1px solid rgba(255, 255, 255, 0.15)",
        borderRadius: `${Math.max(12, baseIconSize * 0.4)}px`,
        boxShadow: `
          0 ${Math.max(4, baseIconSize * 0.1)}px ${Math.max(16, baseIconSize * 0.4)}px rgba(0, 0, 0, 0.4),
          0 ${Math.max(2, baseIconSize * 0.05)}px ${Math.max(8, baseIconSize * 0.2)}px rgba(0, 0, 0, 0.3),
          inset 0 1px 0 rgba(255, 255, 255, 0.15),
          inset 0 -1px 0 rgba(0, 0, 0, 0.2)
        `,
        padding: `${padding}px`,
        width: `${contentWidth + padding * 2}px`,
      }}
      tabIndex={0}
    >
      <div
        className="relative"
        style={{
          height: `${baseIconSize}px`,
          width: "100%",
        }}
      >
        {apps.map((app, index) => {
          const scale = currentScales[index] ?? minScale
          const position = currentPositions[index] ?? 0
          const scaledSize = baseIconSize * scale

          return (
            <div
              className="absolute flex cursor-pointer flex-col items-center justify-end"
              key={app.id}
              onClick={() => handleAppClick(app.id, index)}
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") {
                  e.preventDefault()
                  handleAppClick(app.id, index)
                }
              }}
              ref={(el) => {
                iconRefs.current[index] = el
              }}
              role="button"
              style={{
                bottom: "0px",
                height: `${scaledSize}px`,
                left: `${position - scaledSize / 2}px`,
                transformOrigin: "bottom center",
                width: `${scaledSize}px`,
                zIndex: Math.round(scale * 10),
              }}
              tabIndex={0}
              title={app.name}
            >
              {/* biome-ignore lint/performance/noImgElement: dock icons resize every frame (magnification) and load from a remote CDN under static export (images.unoptimized); next/image adds layout wrappers and offers no optimization here. */}
              <img
                alt={app.name}
                className="object-contain"
                height={scaledSize}
                src={app.icon}
                style={{
                  filter: `drop-shadow(0 ${scale > 1.2 ? Math.max(2, baseIconSize * 0.05) : Math.max(1, baseIconSize * 0.03)}px ${scale > 1.2 ? Math.max(4, baseIconSize * 0.1) : Math.max(2, baseIconSize * 0.06)}px rgba(0,0,0,${0.2 + (scale - 1) * 0.15}))`,
                }}
                width={scaledSize}
              />

              {openApps.includes(app.id) && (
                <div
                  className="absolute"
                  style={{
                    backgroundColor: "rgba(255, 255, 255, 0.8)",
                    borderRadius: "50%",
                    bottom: `${Math.max(-2, -baseIconSize * 0.05)}px`,
                    boxShadow: "0 0 4px rgba(0, 0, 0, 0.3)",
                    height: `${Math.max(3, baseIconSize * 0.06)}px`,
                    left: "50%",
                    transform: "translateX(-50%)",
                    width: `${Math.max(3, baseIconSize * 0.06)}px`,
                  }}
                />
              )}
            </div>
          )
        })}
      </div>
    </div>
  )
}

export { type DockApp, MacOSDock, type MacOSDockProps }

// Demo
const demoApps: DockApp[] = [
  {
    icon: "https://cdn.jim-nielsen.com/macos/1024/finder-2021-09-10.png?rf=1024",
    id: "finder",
    name: "Finder",
  },
  {
    icon: "https://cdn.jim-nielsen.com/macos/1024/calculator-2021-04-29.png?rf=1024",
    id: "calculator",
    name: "Calculator",
  },
  {
    icon: "https://cdn.jim-nielsen.com/macos/1024/terminal-2021-06-03.png?rf=1024",
    id: "terminal",
    name: "Terminal",
  },
  {
    icon: "https://cdn.jim-nielsen.com/macos/1024/mail-2021-05-25.png?rf=1024",
    id: "mail",
    name: "Mail",
  },
  {
    icon: "https://cdn.jim-nielsen.com/macos/1024/notes-2021-05-25.png?rf=1024",
    id: "notes",
    name: "Notes",
  },
  {
    icon: "https://cdn.jim-nielsen.com/macos/1024/safari-2021-06-02.png?rf=1024",
    id: "safari",
    name: "Safari",
  },
  {
    icon: "https://cdn.jim-nielsen.com/macos/1024/photos-2021-05-28.png?rf=1024",
    id: "photos",
    name: "Photos",
  },
  {
    icon: "https://cdn.jim-nielsen.com/macos/1024/music-2021-05-25.png?rf=1024",
    id: "music",
    name: "Music",
  },
  {
    icon: "https://cdn.jim-nielsen.com/macos/1024/calendar-2021-04-29.png?rf=1024",
    id: "calendar",
    name: "Calendar",
  },
]

export function Demo() {
  const [mounted, setMounted] = useState(false)
  const [openApps, setOpenApps] = useState<string[]>(["finder", "safari"])

  useEffect(() => {
    setMounted(true)
  }, [])

  const handleAppClick = (appId: string) => {
    setOpenApps((prev) =>
      prev.includes(appId)
        ? prev.filter((id) => id !== appId)
        : [...prev, appId]
    )
  }

  if (!mounted) {
    return (
      <div className="fixed inset-0 bg-linear-to-b from-blue-400 to-blue-600" />
    )
  }

  return (
    <div className="fixed inset-0 flex items-end justify-center bg-linear-to-b from-blue-400 to-blue-600 pb-4">
      <MacOSDock
        apps={demoApps}
        onAppClick={handleAppClick}
        openApps={openApps}
      />
    </div>
  )
}
